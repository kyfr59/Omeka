var Commenting = {
    ...
    handleReply: function(event) {        
    	//console.log(jQuery(event.target).parent().next().find('p').get(0).tagName);
	...
	moveForm: function(event) {
	    ...
	    // jQuery(event.target).parent().next().find('p')
		jQuery('#comment-form').insertAfter(jQuery(event.target).parent().next());
